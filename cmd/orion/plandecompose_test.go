...
func boundWS(t *testing.T, withTasks bool) *workspace.Workspace {
	t.Helper()
	w := chainWS(t)
	// A real (if minimal) git repo: every caller here runs the chain for
	// real via runPlanChain, and toolkitStep's Frame -- which now writes
	// orion.json directly into RepoDir and stages what specify init wrote
	// (OR-451) -- runs for real too, not stubbed the way okRun replaces
	// supervised stages.
