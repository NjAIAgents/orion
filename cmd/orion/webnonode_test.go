...
func TestBuildAndServeWithNoNodeOnPath(t *testing.T) {
	...
	binDir := t.TempDir()
	binName := "orion-webtest"
	if runtime.GOOS == "windows" {
		// WITHOUT .exe, THIS BINARY IS UNRUNNABLE ON WINDOWS. go build -o
		// happily writes it with no extension, but exec.Command resolving
		// an absolute path still goes through Windows' own executable
		// lookup, which requires .exe (or a name on PATHEXT) -- the exact
		// "executable file not found in %PATH%" this comment now explains,
		// for a file that plainly exists at that exact path.
		binName += ".exe"
	}
