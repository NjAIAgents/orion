// The run view: the card grid (OR-67) and the docked log panel (OR-69),
// rendering /api/snapshot and /api/stream.
//
...
// FILTER_VERBS are the mockup's own chip set: the five verbs, "all", and
// "trace" -- the one word on the panel that is not a verb at all, kept
// distinct in FilterBar rather than folded into VERBS.
...
class FilterBar extends Component {
  render({ active, trace, onToggle, onToggleTrace }) {
    const allOn = VERBS.every((v) => active.has(v));
    return html`
      <div class="filter">
        <button
          class="f ${allOn ? "on" : ""}"
          onClick=${() => this.props.onSetAll(!allOn)}
        >
          all
        </button>
        ${VERBS.map(
...
class LogPanel extends Component {
  ...
  togglePause() {
    this.setState((s) => ({ paused: !s.paused }), () => {
...
// App is the page shell: the topbar, the card grid, and the log panel
// docked beside it.
//
